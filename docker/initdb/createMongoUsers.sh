...
const adminDb = db.getSiblingDB(\"$DB\");
...
$MONGO_CLIENT --port $MONGODB_PORT --eval "
const evseDb = db.getSiblingDB(\"$DB\");
if (!evseDb.getUser(\"$USER\")) {
  evseDb.createUser({
    user: \"$USER\",
    pwd: \"$PASSWORD\",
    roles: [
...
$MONGO_CLIENT --port $MONGODB_PORT --eval "
const defaultUsers = db.getSiblingDB(\"$DB\").getCollection(\"default.users\");
if (!defaultUsers.findOne({ email: \"super.admin@ev.com\" })) {
  defaultUsers.insertOne({
    _id: ObjectId(),
